    ...
    if ( g_variant_type_equal(type, G_VARIANT_TYPE_STRING_ARRAY) ) {
      ...
      for(int i = 0; i < length; i++)
      {
          ...
          if ( element->IsString() ) {
            ...
            element->ToString()->WriteUtf8(val);

            printf("Adding array element: '%s'\n", val);
            string_array[i] = val;
            gboolean is_valid = g_utf8_validate(string_array[i], -1, NULL);

            printf("Is valid UTF8: '%i'\n", is_valid);
            printf("Added array element: '%s'\n", string_array[i]);
          }
          else {
      ...
      printf("Writing array to gsettings.");

      int success = g_settings_set_strv (settings, key, (const gchar * const*)string_array);

      printf("DONE Writing array to gsettings.");
